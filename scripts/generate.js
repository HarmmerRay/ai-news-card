...
function parseArgs() {
  ...
  const opts = { input: null, summary: null, critique: null, date: null, output: null, styles: null };
...
function formatDate(dateStr) {
  ...
  // 支持 ISO 格式 "2026-06-20T04:33:23.000Z" 或 "2026-06-20"
...
function readNewsData(opts) {
  ...
  if (opts.input) {
    ...
    return {
      ...
      date: formatDate(opts.date || data.date || data.pubDate || ''),
    };
  }
...
// ============ HTML 生成 ============
function buildSummaryHTML(text, style, dateStr) {
  const escapeHtml = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const brand = buildBrandHeader(style);
  const deco = style === 'douyin' ? buildDouyinDeco() : buildXHSDeco();
  const dateEl = `<div class="date-label date-${style}">${dateStr}</div>`;
  return `<div class="${style}-summary">${deco}${brand}${dateEl}<div class="text">${escapeHtml(text)}</div></div>`;
}

function buildCritiqueHTML(text, style, dateStr) {
  const escapeHtml = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const brand = buildBrandHeader(style);
  const deco = style === 'douyin' ? buildDouyinDeco() : buildXHSDeco();
  const dateEl = `<div class="date-label date-${style}">${dateStr}</div>`;
  ...
  return `<div class="${style}-critique">${deco}${brand}${dateEl}
